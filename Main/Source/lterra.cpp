...
{
  int flags = MF_ML|GetSpatialFlags();
  if(igraph::noCeiling) flags = MF_ML|MF_FLOOR;
  if(UseBorderTiles())
  {
...
void olterrain::Draw(blitdata& BlitData) const
{
  if(UseBorderTiles())
  {
    ...
    igraph::Blit3(GraphicData.Picture[PictureIndex], BlitData, MF_AL ^ GetSpatialFlags());
  }
  else
  {
    cint AF = GraphicData.AnimationFrames;
    cint F = !(BlitData.CustomData & ALLOW_ANIMATE) || AF == 1 ? 0 : GET_TICK() & (AF - 1);
    igraph::Blit3(GraphicData.Picture[F], BlitData, MF_AL ^ GetSpatialFlags());
  }
}
